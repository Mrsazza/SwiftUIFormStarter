...
struct ContentView: View {
        ...
        NavigationView {
            List {
                ForEach(restaurants) { restaurant in
                    if self.shouldShowItem(restaurent: restaurant) {
            ...
            .navigationBarItems(trailing:
                                    Button(action: {self.showSettings = true}, label: {
    ...
    private func shouldShowItem(restaurent: Restaurant) -> Bool{
        return(!self.settingStore.showCheckInOnly || restaurent.isCheckIn) && (restaurent.priceLevel <= self.settingStore.maxPriceLevel)
    }
}
